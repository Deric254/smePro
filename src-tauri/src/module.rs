...
pub struct ModuleDef {
    ...
    /// Cross-field business rule: `price_floor.price_field` may never
    /// be saved lower than `price_floor.cost_field` on this module.
    /// Optional and backward-compatible, same shape as `date_field`
    /// just above — a module without one (the default) has no such
    /// rule, same as every module before this existed. This replaces
    /// what used to be a rule hardcoded to the literal module id
    /// `"inventory"`: inventory.json now opts in via this instead, and
    /// any OTHER module — built-in or a business's own custom one —
    /// can opt into the identical protection on its own price/cost
    /// pair just by declaring this in its own JSON, no engine change
    /// required. See `check_price_floor` below for where this is
    /// enforced.
    #[serde(default)]
    pub price_floor: Option<PriceFloorRule>,
}

/// See `ModuleDef::price_floor`'s own doc comment above for why this
/// exists and what it replaces.
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct PriceFloorRule {
...
impl ModuleDef {
    ...
    /// Enforces this module's `price_floor` rule (if it declares one)
    /// against `record` — a no-op returning `Ok(())` for any module
    /// that doesn't. `existing` supplies the currently-stored
    /// (price, cost) pair for a PATCH-style partial update, where
    /// `record` may only contain one of the two fields — the caller
    /// passes `None` for a fresh create, where `record` is always
    /// expected to already carry both (defaults having been merged in
    /// upstream), so there's nothing to fall back to.
    pub fn check_price_floor(
        &self,
        record: &std::collections::HashMap<String, Value>,
        existing: Option<(i64, i64)>,
    ) -> Result<()> {
        let Some(rule) = &self.price_floor else { return Ok(()) };
        let (fallback_price, fallback_cost) = existing.unwrap_or((0, 0));
        let price = record.get(&rule.price_field).and_then(|v| v.as_i64()).unwrap_or(fallback_price);
        let cost = record.get(&rule.cost_field).and_then(|v| v.as_i64()).unwrap_or(fallback_cost);
        if price < cost {
            return Err(anyhow!(
